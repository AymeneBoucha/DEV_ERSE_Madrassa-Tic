<template>
  <div class="Signalementtraiter">
    <h1 class="subheading grey--text">Les Annonces Rejetées</h1>
    <v-btn small outlined color="blue" @click="trierAnnonce()" class="mr-2 ARA" dark v-bind="attrs" v-on="on">
              <v-icon left small>check</v-icon>
              <span class="caption text-lowercase">Trier par date</span>
            </v-btn>
            <v-card-actions class="btnsARA">
        ...
        <v-btn outlined color="red" class="btn" to="AnnonceAnnuleAnnoncer">
        ...
        <v-flex v-for="(Annonce, index) in Annonces" :key="Annonce.title">
          <v-card class="text-center card" color="#E5E4E2">
            ...
            <v-card-text class="under">
              ...
              <v-card-text cols="1" class="elseARA">
  ...
  async created() {
    try {
      const res = await axios.get(
        `http://localhost:8080/api/madrasa-tic/announcer/getAllMyRejectedAnnouncementsByAnnouncer`
      );
