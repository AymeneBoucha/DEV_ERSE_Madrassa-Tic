            ...
            <v-card class="text-center cardAS">
              <v-card-text>
                <div class="signalAS">
                  ...
                  ></v-file-input>
                  <v-btn
                    class="enregistrer blue-grey lighten-3"
                    @click="sauvgarder()"
                    ><span>Enregistrer</span></v-btn
                  >
                  <v-btn class="envoie blue-grey darken-2" @click="envoyer()"
...
export default {
  ...
  methods: {
   ...
   data.append('dateOf', this.dateOf)
      ...
      setAuthHeader(acc);
      axios
        .post("http://localhost:8080/api/madrasa-tic/user/saveReport", data)
        .then((res) => {
          console.log(res);
   ...
   data.append('dateOf', this.dateOf)
      ...
      setAuthHeader(acc);
      axios
        .post("http://localhost:8080/api/madrasa-tic/user/submitReport", data)
        .then((res) => {
          console.log(res);
          alert(res.data.message);
          if (res.status == 201) {
