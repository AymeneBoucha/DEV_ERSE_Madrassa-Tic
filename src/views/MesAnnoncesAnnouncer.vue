<template>
  <div class="Signalementtraiter">
    <h1 class="subheading grey--text">Mes Annonces</h1>
    <v-btn small outlined color="blue" @click="trierAnnonce()" class="mr-2 MAA" dark v-bind="attrs" v-on="on">
              <v-icon left small>check</v-icon>
              <span class="caption text-lowercase">Trier par date</span>
            </v-btn>
            <v-card-actions class="btnsMAA">
              <v-btn outlined color="primary" class="btn" to="AjouterAnnonce">
          <v-icon>mdi-plus-box-multiple</v-icon>
          <span>Ajouter une Annonce</span>
        </v-btn>
        <v-btn outlined color="green" class="btn" to="AnnonceExpireAnnoncer">
        ...
        <v-flex v-for="(Annonce, index) in Annonces" :key="Annonce.title">
          <v-card class="text-center cardMAA" color="#E5E4E2">
            ...
            <v-card-text class="under">
              ...
              <v-card-text cols="1" class="elseMAA">
               <v-card-actions class="boutMAA">
                  <v-dialog
                    v-model="dialog3"
                    :retain-focus="false"
                    persistent
              ...
              >
                <template v-slot:activator="{ on }">
                            <v-btn outlined color="red" @click="Annule(index)" class="Sa" dark v-on="on">
                                <span>Annuler</span>
                            </v-btn>
                </template>
                <v-card class="cardT">
                  <v-card-text>
                    <h2 class="subheading grey--text pt-7 text-center">Veuillez préciser la raison de l'annulation ?</h2>
                      <v-card-text>
                      <div class="signal">
                            <v-textarea
                            clearable
                    ...
                    <div class="bouttonsV">
                      ...
                      >
                      <v-btn class="blue-grey darken-2" @click="Annuler()"
...
export default {
  ...
  async created() {
    try {
      const res = await axios.get(
        `http://localhost:8080/api/madrasa-tic/announcer/getAllMyDisplayedAnnouncementsByAnnouncer`
      );
  ...
  methods: {
   ...
   this.titleA = res.data.title;
        ...
        this.pictureA = res.data.picture;
      } catch {
        alert("Missing data from database");
      }
    },
      ...
      try {
       ...
       await axios.post(
          `http://localhost:8080/api/madrasa-tic/announcer/cancelAnnouncementByAnnouncer/${
            this.Annonces[this.varIndex].id
          }`,data
        ),
        this.dialog4 = false;
    ...
    async Annule(index) {
      this.varIndex = index;
        ...
        async Affiche() {
          const res =await  axios.get(`http://localhost:8080/api/madrasa-tic/announcer/getAllMyDisplayedAnnouncementsByAnnouncer`);
          this.Annonces = res.data;
        },
       async  Valide() {
          const res = await axios.get(`http://localhost:8080/api/madrasa-tic/announcer/getAllMyValidatedAnnouncementsByAnnouncer`);
          this.Annonces = res.data;
        },
        async  EnAttente() {
          const res = await axios.get(`http://localhost:8080/api/madrasa-tic/announcer/getAllMyPendingAnnouncementsByAnnouncer`);
          this.Annonces = res.data;
        },
            trierAnnonce(){
