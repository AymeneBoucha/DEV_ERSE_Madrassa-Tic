<template>
  <div class="Signalementtraiter">
    ...
    <v-btn small outlined color="blue" @click="trierAnnonce()" class="mr-2 AAA" dark v-bind="attrs" v-on="on">
              <v-icon left small>check</v-icon>
              <span class="caption text-lowercase">Trier par date</span>
            </v-btn>
            <v-card-actions class="btnsAAA">
        <v-btn outlined color="green" class="btn" to="MesAnnoncesAnnouncer">
          <v-icon>mdi-bullhorn-outline</v-icon>
          <span>Mes Annonces</span>
        </v-btn>
        <v-btn outlined color="orange" class="btn" to="AnnonceExpireAnnoncer">
    ...
    >
      <v-list dense>
        ...
        >
          <v-list-item>
            <v-list-item-content @click="ParMoi()">
              <v-list-item-title >Par Moi</v-list-item-title>
            </v-list-item-content>
          </v-list-item>
          <v-divider vertical></v-divider>
          <v-list-item>
            <v-list-item-content @click="ParRes()">
              <v-list-item-title >Par Le Responsable</v-list-item-title>
            </v-list-item-content>
          </v-list-item>
        </v-list-item-group>
      </v-list>
      ...
      <v-layout row wrap>
        <v-flex v-for="(Annonce, index) in Annonces" :key="Annonce.title">
          <v-card class="text-center cardAAA" color="#E5E4E2">
            <v-responsive >
            ...
            <v-card-text class="under">
              ...
              <v-card-text cols="1" class="elseAAA">
                <v-card-actions class="boutAAA">
                  <v-dialog
                    v-model="dialog3"
                    :retain-focus="false"
                    persistent
...
export default {
  ...
  async created() {
    try {
      const res = await axios.get(
        `http://localhost:8080/api/madrasa-tic/announcer/getAllMyCanceledByMeAnnouncementsByAnnouncer`
      );
  ...
  methods: {
    async Details(index) {
      ...
      async  ParMoi() {
          const res = await axios.get(`http://localhost:8080/api/madrasa-tic/announcer/getAllMyCanceledByMeAnnouncementsByAnnouncer`);
          this.Annonces = res.data;
        },
        async ParRes() {
          const res = await axios.get(`http://localhost:8080/api/madrasa-tic/announcer/getAllMyCanceledByModeratorAnnouncementsByAnnouncer`);
          this.Annonces = res.data;
        },
                trierAnnonce(){
