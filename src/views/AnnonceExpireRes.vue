<template>
  <div class="Signalementtraiter">
    <h1 class="subheading grey--text">Les Annonces Expirées</h1>
     <v-btn small outlined color="blue" @click="trierAnnonce()" class="mr-2 AER" dark v-bind="attrs" v-on="on">
              <v-icon left small>check</v-icon>
              <span class="caption text-lowercase">Trier par date</span>
            </v-btn>
            <v-card-actions class="btnsAER">
  ...
  async created() {
    try {
      const res = await axios.get(
        `http://localhost:8080/api/madrasa-tic/moderator/getAllFinishedAnnouncementsByModerator`
      );
