...
export default {
    data: () => ({
        ...
        which:1,
        links: [
            {icon: 'dashboard', text:'Dashboard', route:'DashboardSer'}, //takes you always to user dashbord (idk how to change it)
            {icon: 'folder', text:'Acceuil', route:'/SigAttent'}, //ajouter path of acceuille 
            {icon: 'person', text:'Profile', route:'/profile'}, //il faut ajouter le path de profile 
        ],
        lns:
        [
            {name:'User', path:'/DashboardUser'}, //dashboard  user
            {name:'Chef de Service', path:'/DashboardChef'}, //dashboard  user
        ],
    }),
    methods: {
        changePath(lns) {
